...
let sp=document.querySelector(".spekar");
...
const animeQuotes = [
  ...
  btn.addEventListener("click",function(){
         Ran=Math.floor(Math.random()*animeQuotes.length);
         Mess.innerHTML=`<p>${animeQuotes[Ran].quote}</p>

          <h1>-<span>${animeQuotes[Ran].character}</span></h1>`

          sp.addEventListener("click",function(){
            var msg = new SpeechSynthesisUtterance();
            
          msg.text = animeQuotes[Ran].quote
